...
switch ($method) {
    case 'GET':
        if (isset($request[2]) && is_numeric($request[2])) {
            echo json_encode($usuario->getByRoleAndId('Candidato', $request[2]));
        } else {
            echo json_encode($usuario->getAllByRole('Candidato'));
        }
        break;
    ...
    case 'POST':
        ...
        if (!$data || !isset($data['nombre'], $data['apellido'], $data['email'], $data['clave'], $data['fecha_nacimiento'], $data['telefono'], $data['direccion'])) {
            ...
            echo json_encode(["message" => "Datos incompletos para crear candidato"]);
            break;
        }
        $data['rol'] = 'Candidato';
    ...
    case 'PATCH':
        if (!isset($request[2]) || !is_numeric($request[2])) {
            http_response_code(400);
            echo json_encode(["message" => "ID de candidato no proporcionado"]);
            break;
        }
        $data = json_decode(file_get_contents("php://input"), true);
